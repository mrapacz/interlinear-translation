...
Copied from transformers.morph.mt5.
"""
...
from transformers import MT5Config, PreTrainedModel, add_start_docstrings
...
from kairos.config import MorphArchitecture as Arch
...
PARALLELIZE_DOCSTRING = r"""
    ...
    Args:
        device_map (`Dict[int, list]`, optional, defaults to None):
            ...
            have fewer attention modules mapped to it than other devices. For reference, the mt5 morph have the
...
# Copied from transformers.morph.t5.modeling_t5.T5LayerNorm with T5->MT5
class MT5LayerNorm(nn.Module):
    def __init__(self, hidden_size, eps=1e-6):
        """
        Construct a layernorm module in the MT5 style.
...
# Copied from transformers.morph.t5.modeling_t5.T5DenseActDense with T5->MT5
...
# Copied from transformers.morph.t5.modeling_t5.T5DenseGatedActDense with T5->MT5
...
# Copied from transformers.morph.t5.modeling_t5.T5LayerFF with T5->MT5
...
# Copied from transformers.morph.t5.modeling_t5.T5Attention with T5->MT5
...
# Copied from transformers.morph.t5.modeling_t5.T5LayerSelfAttention with T5->MT5
...
# Copied from transformers.morph.t5.modeling_t5.T5LayerCrossAttention with T5->MT5
...
# Copied from transformers.morph.t5.modeling_t5.T5Block with T5->MT5
...
# Copied from transformers.morph.t5.modeling_t5.T5PreTrainedModel with T5->MT5, t5->mt5
...
# Copied from transformers.morph.t5.modeling_t5.T5Stack with T5->MT5
class MT5Stack(MT5PreTrainedModel):
    def __init__(self, config, embed_tokens=None):
        ...
        self.is_decoder = config.is_decoder
        morph_config = SINGLETON.config.morph_conf
        if not self.is_decoder:
            # Morphs
            vocabulary_size = get_morph_tokenizer().vocabulary_size
            match morph_config.arch:
                case Arch.SIMPLE_SUM:
                    self.morph_embed_morphs = nn.Embedding(num_embeddings=vocabulary_size, embedding_dim=config.d_model)
                case Arch.AUTOENCODER:
                    self.morph_compress_morphs = nn.Embedding(
                        num_embeddings=vocabulary_size, embedding_dim=morph_config.compressed_embedding_size
                    )
                    self.morph_decompress_morphs = nn.Linear(morph_config.compressed_embedding_size, config.d_model)
                case Arch.CONCATENATE:
                    compressed_word_emb_dim = config.d_model - morph_config.pos_embedding_dim
                    ...
                    self.morph_embed_morphs = nn.Embedding(
                        num_embeddings=vocabulary_size, embedding_dim=morph_config.pos_embedding_dim
                    )
                case _:
                    assert False
    ...
    def forward(
        ...
        if input_ids is not None and inputs_embeds is not None:
            ...
            if self.is_decoder:
                ...
                input_shape = input_ids.size()
                input_ids = input_ids.view(-1, input_shape[-1])  # MR: this flattens the input tensor so it's
        ...
        if inputs_embeds is None:
            ...
            if not self.is_decoder:
                ...
                assert (morph_config := SINGLETON.config.morph_conf) is not None
...
class MT5Model(MT5PreTrainedModel):
    ...
    # Copied from transformers.morph.t5.modeling_t5.T5Model.__init__ with T5->MT5
    def __init__(self, config: MT5Config):
        super().__init__(config)
    ...
    # Copied from transformers.morph.t5.modeling_t5.T5Model.forward with T5->MT5, t5->mt5
    def forward(
        self,
...
class MT5ForConditionalGeneration(MT5PreTrainedModel):
    ...
    def zero_grad(self, set_to_none: bool = True) -> None:
        if not hasattr(self, "_debug_out_dir") or not hasattr(self, "_debug_global_step "):
            super().zero_grad(set_to_none=set_to_none)
            return

        def save_grads(name: str) -> None:
            step = self._debug_global_step
            (Path(self._debug_out_dir) / "grads").mkdir(exist_ok=True)
            self._tb_writer: SummaryWriter
            if step % self._save_grads_every == 0:
                if weights.grad is not None and step not in self.logged_grads:
                    logger.info(f"Logging grads for {step = }")
                    self.logged_grads.add(step)

                    torch.save(weights.grad, (Path(self._debug_out_dir) / "grads" / f"{name}_{str(step).zfill(5)}.pt"))
                    self._tb_writer.add_image(
                        tag="morph_grads",
                        img_tensor=weights.grad,
                        global_step=step,
                        dataformats="HW",
                    )
                else:
                    logger.warning(f"Empty grad for {step = }")

        if not hasattr(self, "logged_grads"):
            self.logged_grads: set[int] = set()

        for layer, weights in self.named_parameters():
            if layer in ("encoder.embed_morphs.weight", "shared.weight"):
                save_grads(layer)

        super().zero_grad(set_to_none=set_to_none)

    # Copied from transformers.morph.t5.modeling_t5.T5ForConditionalGeneration.__init__ with T5->MT5
    def __init__(self, config: MT5Config):
        super().__init__(config)
        ...
        decoder_config.is_encoder_decoder = False
        decoder_config.num_layers = config.num_decoder_layers
        self.decoder = MT5Stack(decoder_config, embed_tokens=self.shared)
    ...
    # Copied from transformers.morph.t5.modeling_t5.T5ForConditionalGeneration.forward with T5->MT5, t5->mt5
    def forward(
        self,
        input_ids: torch.LongTensor | None = None,
...
class MT5EncoderModel(MT5PreTrainedModel):
    ...
    # Copied from transformers.morph.t5.modeling_t5.T5EncoderModel.__init__ with T5->MT5
    def __init__(self, config: MT5Config):
        super().__init__(config)
        self.shared = nn.Embedding(config.vocab_size, config.d_model)
    ...
    @add_start_docstrings(PARALLELIZE_DOCSTRING)
    # Copied from transformers.morph.t5.modeling_t5.T5EncoderModel.parallelize
    def parallelize(self, device_map=None):
        self.device_map = (
            get_device_map(len(self.encoder.block), range(torch.cuda.device_count())) if device_map is None else device_map
        )
    ...
    @add_start_docstrings(DEPARALLELIZE_DOCSTRING)
    # Copied from transformers.morph.t5.modeling_t5.T5EncoderModel.deparallelize
    def deparallelize(self):
        self.encoder.deparallelize()
        self.encoder = self.encoder.to("cpu")
        self.model_parallel = False
    ...
    # Copied from transformers.morph.t5.modeling_t5.T5EncoderModel.get_input_embeddings
    def get_input_embeddings(self):
        return self.shared

    # Copied from transformers.morph.t5.modeling_t5.T5EncoderModel.a
    def set_input_embeddings(self, new_embeddings):
        self.shared = new_embeddings
        self.encoder.set_input_embeddings(new_embeddings)

    # Copied from transformers.morph.t5.modeling_t5.T5EncoderModel.get_encoder
    def get_encoder(self):
        return self.encoder

    # Copied from transformers.morph.t5.modeling_t5.T5EncoderModel._prune_heads
    def _prune_heads(self, heads_to_prune):
        """
        Prunes heads of the model.
    ...
    # Copied from transformers.morph.t5.modeling_t5.T5EncoderModel.forward with T5->MT5, t5->mt5
    def forward(
        self,
        input_ids: torch.LongTensor | None = None,
        attention_mask: torch.FloatTensor | None = None,
